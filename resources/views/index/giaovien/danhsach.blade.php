...
@section('title')
    <title>Danh sách giáo viên - Quản lý học sinh</title>
@endsection
@section('style')
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="form-group">
                <a href="{{route('them-giaovien.get')}}">
                    <button class="ladda-button btn btn-default waves-effect waves-light" data-style="expand-right">
                        <span class="btn-label"><i class="fa fa-plus"></i></span>Thêm giáo viên
                    </button>
                </a>
            </div>
        </div>
                ...
                <table id="datatable-responsive"
                    ...
                    @foreach($dsGV as $i => $detail)
                        <tr>
                            ...
                            <td>{{$detail->BangCap}}</td>
                            <td>{{$detail->ChuyenMon}}</td>
                            <td>
                                <a href="{{route("sua-giaovien.get",[$detail->MaGV])}}"
                                   class="btn btn-icon waves-effect waves-light btn-warning" title="Sửa"><i
                                            class="fa fa-wrench"></i></a>
                                &nbsp;
                                &nbsp;
                                <a onclick="del({{$detail->MaGV}})"
...
@section('script')
    <script>
        function del(id) {
            $.confirm({
                text: "Hành động này sẽ xóa dữ liệu của giáo viên này. Bạn có chắc muốn xóa không?",
                title: "Xác nhận xóa",
                ...
                confirm: function (button) {
                    window.location.assign("giaovien/xoa/" + id);
                },
